...
export default async function userHandler(
  ...
  switch (method) {
    case "PUT":
      console.log("RESETTING PACMAN FOR ALL PLAYERS");
      ...
      allPlayers
        ...
        .filter((p) => !!p.details?.pacmanDetails)
        .forEach(async (player) => {
          const playerDetails: PlayerDetails = {
            ...player.details,
            pacmanPlayer: false,
          };
          delete playerDetails.pacmanDetails;

          await updatePlayer(player.id, playerDetails).then(() =>
            console.info("Reset player pacman details", player.id)
          );
        });
